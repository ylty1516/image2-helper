...
```text
Anime-real integration support: lock the existing real background plate first. Preserve every background object, object position, crop, camera perspective, exposure, color temperature, original light, original shadows, highlights, reflections, clutter, grain, and compression. Do not clean, relight, repaint, stylize, replace, blur, sharpen, denoise, rearrange, add, remove, or move anything in the background. Before generating the character layer, define physical anchors (seat/desk/floor/screen plane, hand/foot/body contact, support points, occlusion order) and a background lighting map (main light, fill light, lit side, shadow side, contact-shadow zones, bounce color, exposure range). Then match the anime character layer to the real camera plate, scale anchors, horizon/ground plane, light map, edge softness, lens grain/compression, and foreground occlusion. Avoid floating sticker cutout, hands hovering above target objects, mismatched lighting, wrong scale, pure cel colors unaffected by the room, changed background objects, changed room lighting, and clean pasted edges.
```
